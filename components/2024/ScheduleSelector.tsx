...
import selectionSchemes, {
  SelectionSchemeType,
  SelectionType,
} from "./selection-schemes";
...
> {
  ...
  handleSelectionStartEvent(startTime: Date) {
    ...
    const timeSelected = this.props.selection.find((a) =>
      isSameMinute(a, startTime)
    );
    this.setState({
  ...
  renderDateCellWrapper = (time: Date): JSX.Element => {
    ...
    const selected = Boolean(
      this.state.selectionDraft.find((a) => isSameMinute(a, time))
    );
  ...
  renderDateLabel = (date: Date): JSX.Element => {
    if (this.props.renderDateLabel) {
      ...
      return <DateLabel>{formatDate(date, this.props.dateFormat?.replace('D', 'd') || 'MM/dd')}</DateLabel>;
    }
  };
